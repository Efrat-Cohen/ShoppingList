...
In the full stack this service sits behind the BFF, which resolves product details before
calling it, and is not published to the host. The compose file here runs it on its own, with
its port exposed, so you can call it directly. It validates its own input either way.
